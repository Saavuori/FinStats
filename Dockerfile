...
RUN CGO_ENABLED=0 GOOS=linux go build \
    -ldflags="-s -w -X finstats/internal/api.Version=${VERSION} -X finstats/internal/api.BuildDate=${BUILD_DATE} -X finstats/internal/api.GitCommit=${GIT_SHA}" \
    -o finstats ./cmd/server

# Stage 3: Minimal runtime
FROM alpine:3.21
RUN apk --no-cache add ca-certificates tzdata
WORKDIR /app
COPY --from=backend-builder /app/finstats .
ENV PORT=8080
EXPOSE 8080
ENTRYPOINT ["./finstats"]
